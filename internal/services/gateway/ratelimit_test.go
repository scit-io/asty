...
func TestRL_LRUEvictsOldestWhenFull(t *testing.T) {
	...
	// 4-й IP — должен вытеснить 1.1.1.1 (back).
...
func TestRL_CleanupExpired(t *testing.T) {
	...
	// Список после трёх get: front=fresh, middle=old2, back=old1.
	// cleanupExpired идёт с back, останавливается на первой свежей.
...
func TestRL_CleanupStopsAtFirstFresh(t *testing.T) {
	...
	// LRU-инвариант: back имеет минимальный lastSeen. Если в back свежая запись —
	// все остальные тоже свежие, cleanup останавливается сразу.
	r.get(r.general, "fresh", 100, 10)
	r.general.m["fresh"].lastSeen = now
