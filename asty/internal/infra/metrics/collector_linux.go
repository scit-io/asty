...
func (c *Collector) getCPUPercent(pid int) (float64, error) {
	...
	c.mu.Lock()
	prev, hasPrev := c.prevTicks[pid]
	c.prevTicks[pid] = totalTicks
	c.mu.Unlock()
	...
	clkTck := uint64(100)
	intervalSec := uint64(c.interval.Seconds())
	if intervalSec == 0 {
		intervalSec = 1
	}
...
func (c *Collector) getMemoryMB(pid int) (int64, error) {
