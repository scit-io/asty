...
// nats-server 2.14.2 confirms that Update CLEARS the per-key TTL
// entirely — the nats.go comment "Update also resets the TTL associated
// with the key" means "removes it", not "re-arms the timer". So
// heartbeat-renewal must publish directly on $KV.<bucket>.<key> with
// Nats-TTL set on every write.
//
