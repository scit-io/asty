...
// Service is the SSE companion to GET /dashboard/v1/services/{name}.
func Service(ctx Context, w http.ResponseWriter, r *http.Request, serviceName string) {
	if serviceName == "" {
...
func emitServiceView(w http.ResponseWriter, snap *types.ClusterSnapshot, name string) {
	...
	Event(w, "detail", types.MustJSON(map[string]any{
